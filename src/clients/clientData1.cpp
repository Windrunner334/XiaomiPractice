        ...
        char** argv)
{
    ...
    const char* topic_xml = "<dds>"
            "<topic>"
            "<name>InfoStudentTopic</name>"
            "<dataType>InfoStudent</dataType>"
            "</topic>"
            "</dds>";
    ...
    {
        InfoStudent topic = {
            "huxiao", rand()%100000, 1l
        };

        ucdrBuffer ub;
        uint32_t topic_size = InfoStudent_size_of_topic(&topic, 0);
        InfoStudent_serialize_topic(&ub, &topic);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &ub, topic_size);

        printf("Student name: %s, number: %d, grade: %d\n", topic.name, topic.number, topic.grade);
