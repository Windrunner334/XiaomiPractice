            ...
            eprosima::uxr::TransportRc& transport_rc) -> ssize_t
    {
        ...
        if (0 < poll_rv)
        {
            ...
            if(bytes_received != -1){
                ...
                if(InfoStudent_deserialize_topic(&ub, &topic)){
                    stuVec.push_back(student{topic.name, topic.number, topic.grade});
                    /*
                    UXR_AGENT_LOG_INFO(
                        UXR_DECORATE_RED(
                            "Message from client"),
